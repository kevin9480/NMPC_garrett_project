...
**mpc.R_du** — 값이 클수록 제어 경로가 완만해집니다(제어 궤적을 부드럽게 만듦).

![R_du 파라미터 비교](assets/param_R_du_compare.png)

**mpc.Q_r** — 값이 클수록 그래프 교차점이 왼쪽으로 이동하고, 모터 사용이 감소합니다.
...
먼저 과제에서 주어진 목표 응답 패턴을 재현하는 것을 1차 목표로 삼아, 기본 세팅(`R_du=0.5`, `Q_r=0.01`)에서 `R_du`를 낮춰가며(0.5 → 0.05 → 0.01) 제어 궤적을 목표에 가깝게 맞췄습니다.

![과제 목표 구현 비교](assets/goal_match_compare.png)
...
이 조사를 근거로, **SOC를 40~60% 제한 구간 내에서 50% 안팎으로 유지하는 것**을 최적화 목표로 설정하고, `mpc.Q_r`을 0.01 → 50으로 크게 높여 SOC 추종을 강화하는 방향으로 파라미터를 재설계했습니다.
...
교내 Garrett Motion NMPC 제어기 설계 경진대회 **우수상** 수상
